...
-- 创建索引
CREATE INDEX idx_mv_g1_ketto ON mv_g1_horse_records(ketto_num);
CREATE INDEX idx_mv_g1_race ON mv_g1_horse_records(race_date, jyo_cd, kaiji, nichiji, race_num, umaban);
CREATE INDEX idx_mv_g1_rank ON mv_g1_horse_records(kakutei_jyuni);

-- =====================================================
-- 2. 物化视图：预计算所有马匹组合的共同参赛次数
--    这是最耗时的 self-join，预先计算后查询只需毫秒级
-- =====================================================
CREATE MATERIALIZED VIEW mv_g1_horse_pairs AS
SELECT
    r1.ketto_num AS source,
    r2.ketto_num AS target,
    COUNT(*) AS weight
FROM mv_g1_horse_records r1
JOIN mv_g1_horse_records r2 ON r1.race_date = r2.race_date
...
-- 创建索引
CREATE INDEX idx_mv_pairs_source ON mv_g1_horse_pairs(source);
CREATE INDEX idx_mv_pairs_target ON mv_g1_horse_pairs(target);
CREATE INDEX idx_mv_pairs_weight ON mv_g1_horse_pairs(weight);
